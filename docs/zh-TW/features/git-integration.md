...
無需離開 PackageFlow 即可進行視覺化 Git 操作。暫存、提交、分支、差異比對 — 全部無需 CLI 體操。
...
PackageFlow 顯示字元數並在您的訊息有以下情況時警告：
- 太短（少於 10 個字元）
- 第一行太長（超過 72 個字元）
...
PackageFlow 對大型差異使用虛擬化，即使有數千行也能保持效能流暢。
