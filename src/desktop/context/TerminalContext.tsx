...
export function TerminalProvider({ children }: { children: ReactNode }) {
  ...
  for (const [pid, ss] of sessionStore) {
    hasUnreadByProject.set(pid, ss.some(s => s.hasUnread));
  }
  ...
  const spawnSession = useCallback(async (command?: string, args?: string[], name?: string) => {
    const id = crypto.randomUUID();
    const sessionName = name ?? (command ? command.split(' ').pop()! : `Shell ${(sessionStore.get(currentProjectId)?.length ?? 0) + 1}`);
  ...
  const mountSession = useCallback((id: string, container: HTMLElement) => {
    const live = liveStoreRef.current.get(id);
